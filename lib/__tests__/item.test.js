...
it('gives automatic accessibility id based on content', () => {
    const content = 'This is wonderfull content'
    const wrapper = TestRenderer.create(<Item content={content} />);
    const text = wrapper.root.findByProps({ testID: 'container-item' })
    expect(text.props.accessibilityLabel).toBe('picker_item:' + 'This is wonderfull content')
})
