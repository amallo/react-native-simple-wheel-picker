...
it('scrolls to selected item position when item selected', () => {
  ...
  let wrapper
  act(() => {
    wrapper = TestRenderer.create(<WheelPicker onSelected={onSelected} items={["P0", "P1", "P2", "P3"]} selectedIndex={selectedIndex} />);
  })
  const visibleItems = wrapper.root.findAllByProps({
    testID: 'item'
  })
...
it('automatically select item on scroll', () => {
  let wrapper
  act(() => {
    wrapper = TestRenderer.create(<WheelPicker itemHeight={33} items={["P0", "P1", "P2", "P3"]} />);
  })

  const onSelected = jest.fn()
  const scrollPosition = 200
  act(() => {
    wrapper.update(<WheelPicker onSelected={onSelected} />)
  ...
  expect(onSelected).toBeCalledTimes(1)
  expect(onSelected).toBeCalledWith(Math.round(scrollPosition / 33))
})

it('selects the first item at initialization', () => {
  const onSelected = jest.fn()
  let wrapper
  act(() => {
  ...
  expect(onSelected).toBeCalledTimes(1)
  expect(onSelected).toBeCalledWith(0)
})
