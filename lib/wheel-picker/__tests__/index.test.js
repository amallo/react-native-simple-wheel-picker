...
it('renders a selected style for text items', () => {
  const data = ["A", "B"]
  const selectedIndex = 0
  const selectedItemTextStyle = { color: 'red' }
  let wrapper
  act(() => {
    wrapper = TestRenderer.create(<WheelPicker selectedIndex={selectedIndex} selectedItemTextStyle={selectedItemTextStyle} items={data} />);
  })
  const items = wrapper.root.findAllByProps({
    testID: 'item'
  })
  expect.assertions(data.length)
  items.forEach((item) => expect(item.props.selectedTextStyle).toEqual(selectedItemTextStyle))
})
it('renders a default selected style for text items', () => {
  const data = ["A", "B"]
  const selectedIndex = 0
  let wrapper
  act(() => {
    wrapper = TestRenderer.create(<WheelPicker selectedIndex={selectedIndex} items={data} />);
  })
  const items = wrapper.root.findAllByProps({
    testID: 'item'
  })
  expect.assertions(data.length)
  items.forEach((item) => expect(item.props.selectedTextStyle).toEqual(defaultSelectedTextStyle))
})
it('renders a unselected style for text items', () => {
  const data = ["A", "B"]
  const selectedIndex = 0
  const unselectedItemTextStyle = { color: 'red' }
  let wrapper
  act(() => {
    wrapper = TestRenderer.create(<WheelPicker selectedIndex={selectedIndex} unselectedItemTextStyle={unselectedItemTextStyle} items={data} />);
  })
  const items = wrapper.root.findAllByProps({
    testID: 'item'
  })
  expect.assertions(data.length)
  items.forEach((item) => expect(item.props.unselectedTextStyle).toEqual(unselectedItemTextStyle))
})
it('renders a default unselected style for text items', () => {
  const data = ["A", "B"]
  const selectedIndex = 0
  let wrapper
  act(() => {
  ...
  expect.assertions(data.length)
  items.forEach((item) => expect(item.props.unselectedTextStyle).toEqual(defaultUnselectedTextStyle))
})

describe("accessibility labels", () => {
  it('renders an accessibility label for the container', () => {
    const data = ["A", "B"]
    let wrapper
    let accessibilityLabel = "picker1"
    act(() => {
      wrapper = TestRenderer.create(<WheelPicker accessibilityLabel={accessibilityLabel} items={data} />);
    })
    const container = wrapper.root.findByProps({ testID: 'container' })
    ...
    items.forEach((item) => expect(item.props.accessibilityLabel).toEqual(`${accessibilityLabel}:${item.props.content}`))
  })

  it('renders a default accessibility label', () => {
    const data = ["A", "B"]
    let wrapper
    act(() => {
      wrapper = TestRenderer.create(<WheelPicker items={data} />);
    })
    const container = wrapper.root.findByProps({ testID: 'container' })
    expect(container.props.accessibilityLabel).toBe(defaultAccessibilityLabel)

    const items = wrapper.root.findAllByProps({
      testID: 'item'
    })
    expect.assertions(data.length + 1)
    items.forEach((item) => expect(item.props.accessibilityLabel).toEqual(`${defaultAccessibilityLabel}:${item.props.content}`))
  })
})